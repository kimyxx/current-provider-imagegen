...
The default Images API model is `gpt-image-2`. Use `--model` only when the provider exposes a different compatible image model.
...
- Default `--api-mode responses` sends `POST <base_url>/v1/responses` with `stream: true` and an `image_generation` tool. The tool receives `partial_images` for progressive previews.
